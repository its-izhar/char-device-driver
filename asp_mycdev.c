...
/* Helper function for init to setup cdev struct in asp_mycdev */
...
static int mycdev_init_module(void)
{
	...
	printk(KERN_INFO "Initializing Module: ASP_MYCDEV!\n");
	...
	if(retval < 0){
		printk(KERN_WARNING "Unable to allocate major: %d", mycdev_major);
		return retval;
	}
	...
	for(i = 0; i < max_devices; i++)
	{
		...
		/* TODO:: Setup cdev struct here */
		setup_cdev(&mycdev_devices[i], i);
	}
	/* cleanup if we failed to allocate device memory */
	if(ramdiskAllocFailed == true){
		printk(KERN_WARNING "Unable to allocate device memory: %d", ramdiskFailDevIndex);
		retval = -ENOMEM;
		goto FAIL;
	}
...
static void mycdev_cleanup_module(void)
{
	...
	printk(KERN_INFO "Cleaning Up Module: ASP_MYCDEV!\n");
